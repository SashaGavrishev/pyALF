...
def check_warmup_ipy(directories, names, custom_obs=None, ncols=3):
    ...
    """
    return CheckWarmupIpy(directories, names, custom_obs=custom_obs, ncols=ncols).gui
...
class CheckWarmupIpy:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, directories, names, custom_obs=None, ncols=3):
        self.gui, self.log, self.axs, self.nmax, self.nskip, self.select = init_layout(
            directories,
            n_plots=len(names),
            ncols=ncols,
    ...
    def _init_dir(self):
        ...
        with self.log:
            self.bins = _get_bins(self.select.value, self.names, self.custom_obs)
