...
def analysis(directory, symmetry=None, custom_obs=None, do_tau=True, always=False):
    ...
    if "data.h5" in os.listdir(directory):
        if not always:
            try:
                d1 = os.path.getmtime(
                    os.path.join(directory, "data.h5")
    ...
    if custom_obs is not None:
        ...
        for obs_name, obs_spec in custom_obs.items():
            if all(x in list_obs for x in obs_spec["needs"]):
                ...
                jacks = [ReadObs(directory, obs_name) for obs_name in obs_spec["needs"]]
                ...
                for i in range(N_bins):
                    J[i] = obs_spec["function"](
                        *[x for j in jacks for x in j.slice(i)], **obs_spec["kwargs"]
                    )
                ...
                np.savetxt(os.path.join(directory, "res", obs_name), dat)
    ...
    for obs_name in list_eq:
        print(obs_name)
        try:
            sign, m_k, e_k, m_k_sum, e_k_sum, m_r, e_r, m_r_sum, e_r_sum, latt = ana_eq(
                directory, obs_name, sym=symmetry
            )
        except TooFewBinsError:
            print("Too few bins, skipping.")
            continue
    ...
    if do_tau:
        ...
        for obs_name in list_tau:
            print(obs_name)
            try:
                sign, m_k, e_k, m_r, e_r, dtau, latt = ana_tau(
                    directory, obs_name, sym=symmetry
                )
            except TooFewBinsError:
                print("Too few bins, skipping.")
            ...
            write_res_tau(directory, obs_name, m_k, e_k, m_r, e_r, dtau, latt)
