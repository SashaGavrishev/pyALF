...
def _get_arg_parser():
    ...
    parser.add_argument(
        ...
        "--rebin",
        action="store_true",
        help="Check rebinning for controlling autocorrelation. Opens new window.",
    )
    parser.add_argument(
        "-l",
        "--check_list",
    ...
    parser.add_argument(
        "--do_analysis", "--ana", action="store_true", help="Do analysis."
    )
    parser.add_argument(
        "--always",
        action="store_true",
        help="Do not skip analysis if parameters and bins are older than results.",
    )
    parser.add_argument(
        "--gather",
        action="store_true",
    ...
    parser.add_argument(
        "--no_tau", action="store_true", help="Skip time displaced correlations."
    )
    parser.add_argument(
        "--custom_obs",
...
def _main():
    ...
    if args.symmetry is None:
        ...
        try:
            symmetry_mod = import_module("symmetry", os.path.expanduser(args.symmetry))
        except FileNotFoundError as symmetry_not_found:
            raise FileNotFoundError(
                f'"{args.symmetry}" not found.'
