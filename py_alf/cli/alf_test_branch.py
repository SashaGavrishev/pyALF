...
def _prepare_runs(tmpdir, sim_R, sim_T):
    executable_R = os.path.join(
        tmpdir, f"ALF_{sim_R.config.replace(' ', '_')}_reference.out"
    )
    ...
    executable_T = os.path.join(
        tmpdir, f"ALF_{sim_T.config.replace(' ', '_')}_test.out"
    )
...
def _get_arg_parser():
    ...
    parser.add_argument(
        "--n_mpi", default=4, help="Number of MPI processes.               (default: 4)"
    )
    parser.add_argument(
        "--mpiexec",
        default="mpiexec",
    ...
    parser.add_argument(
        "--mpiexec_args", default="", help="Additional arguments to MPI executable."
    )
    parser.add_argument(
        "--no_prep",
        action="store_true",
    ...
    parser.add_argument("--no_sim", action="store_true", help="Do not run ALF binary.")
    parser.add_argument(
        "--no_analyze", action="store_true", help="Do not analyze and compare results."
    )
    return parser
