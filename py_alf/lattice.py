...
class UnitCell:
    ...
    def __init__(self, latt_grp):
        ...
        if self.Norb != latt_grp.attrs["Norb"]:
            ...
            print(
                f"Wrong values Norb={latt_grp.attrs['Norb']}, "
                f"N_coord={latt_grp.attrs['N_coord']}"
            )
            print(
                f"Correct values Norb={latt_grp.attrs['N_coord']}, "
    ...
    *args : dict, tuple, or list
        ...
        if s in _cache:
            ...
            if _use_fortran_init and (not force_python_init):
                try:
                    ...
                    print(
                        "Compiled Fortran lattice initialisazion does not work, "
    ...
    def plot_r(self, data):
        ...
        cmap = mpl.cm.ScalarMappable(
            mpl.colors.Normalize(vmin=data.min(), vmax=data.max()), mpl.cm.Greys
...
def _plot_2d(coords, vec1, vec2, ax, data, cmap):
    ...
    codes = [Path.MOVETO] + (len(verts0) - 2) * [Path.LINETO] + [Path.CLOSEPOLY]

    for coord, dat in zip(coords, data):
        verts = verts0 + coord
        ax.add_patch(
            mpl.patches.PathPatch(Path(verts, codes), facecolor=cmap.to_rgba(dat), lw=0)
        )
...
def _init0(L1, L2, a1, a2):
    ...
    lattices_fortran = CDLL(os.path.join(os.path.dirname(__file__), lattices_bin))
    ndim = c_int(len(L1))
...
def _init1(L1, L2, a1, a2):
    ...
    mat2 = 2.0 * np.pi * np.linalg.inv(mat)
    BZ1 = np.copy(mat2[:, 0])
    BZ2 = np.copy(mat2[:, 1])
    ...
    X = (
        2.0
        * np.pi
        / (np.dot(BZ1, L1) * np.dot(BZ2, L2) - np.dot(BZ2, L1) * np.dot(BZ1, L2))
    )
    ...
    N1 = abs(int(round(np.dot(BZ1, L1) / (2.0 * np.pi))))
    N2 = abs(int(round(np.dot(BZ2, L1) / (2.0 * np.pi))))
    N3 = abs(int(round(np.dot(BZ1, L2) / (2.0 * np.pi))))
    N4 = abs(int(round(np.dot(BZ2, L2) / (2.0 * np.pi))))
    ...
    for i1 in range(-L, L + 1):
        for i2 in range(-L, L + 1):
            ...
            for a in [L2, L1, L2 - L1, L2 + L1]:
                if (np.dot(x, a) > np.dot(a, a) / 2.0 + 1e-5) or (
                    np.dot(x, a) < -np.dot(a, a) / 2.0 + 1e-5
                ):
                    in_latt = False
            if in_latt:
                invlistr[i1, i2] = N
    ...
    for i1 in range(-L, L + 1):
        for i2 in range(-L, L + 1):
            ...
            for b in [BZ2, BZ1, BZ2 - BZ1, BZ2 + BZ1]:
                if (np.dot(x, b) > (np.dot(b, b) / 2.0 + 1e-7)) or (
                    np.dot(x, b) < (-np.dot(b, b) / 2.0 + 1e-7)
                ):
                    in_latt = False
            if in_latt:
                listk[nc] = [i1, i2]
    ...
    for n in range(N):
        for nd1 in [-1, 0, 1]:
            for nd2 in [-1, 0, 1]:
                ...
                n1 = int(round(np.dot(BZ1, x) / (2.0 * np.pi)))
                n2 = int(round(np.dot(BZ2, x) / (2.0 * np.pi)))
                nn = invlistr[(n1, n2)]
    ...
    for i in range(N):
        ...
        for j in range(N):
            ...
            n1 = int(round(np.dot(BZ1, d) / (2.0 * np.pi)))
            n2 = int(round(np.dot(BZ2, d) / (2.0 * np.pi)))
